        ...
        return (
            <div className='pageContainer'>
                <div className={`${classes.statisticsContainer} pt-card pt-elevation-1`}>
                    ...
                    {stats.named_entities.total > 0 && <div className={classes.statisticsItem}>
                        <h3>{localization.searchPage.namedEntitiesLabel}&nbsp;({stats.named_entities.data.length})</h3>
                        {stats.named_entities.data.map((ne, id) => (
                            <TagsWithCount
                                key={id}
                                label={ne.name}
                                onClick={(() => performSearchByNamedEntity(ne.name))}
                                count={`${ne.type} | ${ne.hits_percent.toFixed(2)}%`} />
                        ))}
                    </div>} 
                </div>
            </div>
